...
import { useGetPrinterSettings } from "@/lib/api-client-react";
...
function formatDate(dateStr: string) {
  ...
  return { date: `${day}/${month}/${year}`, time: `${hours}:${mins}:${secs} ${ampm}` };
}
...
export function MasterReceiptSlip({
  ...
  const { data: qPrinterSettings } = useGetPrinterSettings();
  const ps = printerSettings || qPrinterSettings;

  const paperWidth = ps?.paperWidth ?? 80;
  const leftMargin = ps?.leftMargin ?? 4;
  const rightMargin = ps?.rightMargin ?? 4;
  const topMargin = ps?.topMargin ?? 2;
  const bottomMargin = ps?.bottomMargin ?? 2;
  const fontSize = ps?.fontSize ?? 10;
  const lineSpacing = ps?.lineSpacing ?? 2;
  ...
  return (
    <div className="receipt-slip" style={slipStyle} dir="rtl">
      ...
      {/* Visual Cut Indicator & Bottom Feed Spacing */}
      <div className="receipt-divider-dots my-4" />
      <div className="text-center text-xs font-black text-slate-400 select-none tracking-widest my-1">
        ✂ - - - - - - - - - - - - - - - - - - - - - - - - ✂
      </div>
      <div className="receipt-divider-dots my-4" />
      <div className="h-12" />
...
export function DeptReceiptSlip({
  ...
  return (
    <div className="receipt-slip receipt-dept-slip" style={slipStyle} dir="rtl">
      ...
      <table className="dept-slip-table w-full border-collapse border border-black" style={{ tableLayout: 'fixed' }}>
        <colgroup>
          <col style={{ width: "75%" }} />
          <col style={{ width: "25%" }} />
        </colgroup>
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx} className="border-b border-black last:border-b-0">
              ...
              <td className="text-center font-black text-3xl border-r border-black bg-gray-50 p-1">
...
export function ReceiptPreview({
  ...
  deptGroups = [],
  printerSettings,
}: ReceiptProps & { copyLabels?: string[]; printerSettings?: any }) {
  const { data: qPrinterSettings } = useGetPrinterSettings();
  const ps = printerSettings || qPrinterSettings;
