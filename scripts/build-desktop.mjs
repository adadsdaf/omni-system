...
function ensureDirs() {
  ...
  mkdirSync(join(electronAppDist, "splash"), { recursive: true });
}
...
function copySplashLogo() {
  const logoSrc = join(frontendDistTarget, "omnisystem-logo.png");
  const logoTarget = join(electronAppDist, "splash", "omnisystem-logo.png");
  if (existsSync(logoSrc)) {
    mkdirSync(join(electronAppDist, "splash"), { recursive: true });
    copyFileSync(logoSrc, logoTarget);
    console.log("  Splash logo copied to dist");
  } else {
    console.warn("  Splash logo not found in frontend dist, skipping.");
  }
}
...
function main() {
  ...
  console.log("\nStep 6/6: Copying splash assets...");
  copySplashLogo();

  console.log("\nStep 7/7: Copying Electron source to dist...");
  copySrcToDist();
